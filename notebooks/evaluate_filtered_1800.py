...
HORIZON     = 1800     # seconds — must match the saved model
...
def main():
    ...
    # Truncate true values to match the generator's exact output length
    valid_len_full = len(y_pred_full_log1p)
    y_true_full_kwh = np.expm1(np.clip(y_test[:valid_len_full].astype(np.float64), -10., 30.))
    ...
    # ── 6. Predict on CLEAN test set (artefacts removed) ─────────────────────
    print("\nPredicting on CLEAN test set (artefacts removed) ...")
    clean_gen = SequenceDataGenerator(
        X_test_clean, y_test_clean, batch_size=BATCH_SIZE, shuffle=False
    )
    y_pred_clean_log1p = model.predict(clean_gen, verbose=1).flatten()

    # Truncate true values to match the generator's exact output length
    valid_len_clean = len(y_pred_clean_log1p)
    y_true_clean_kwh = np.expm1(np.clip(y_test_clean[:valid_len_clean].astype(np.float64), -10., 30.))
    
    y_pred_clean_kwh = np.maximum(
        np.expm1(np.clip(y_pred_clean_log1p.astype(np.float64), -10., 30.)),
        0.0
    )
