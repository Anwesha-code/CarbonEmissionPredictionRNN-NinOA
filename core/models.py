...
Revision notes (v2)
--------------------
The loss function has been changed from 'mse' to a configurable option
(default: Huber loss with delta = 1.0).  This change addresses the model
collapse observed in the first training run.

Justification: after log1p transformation the target distribution is
approximately Gaussian, but a residual right tail persists.  Huber loss
is quadratic for residuals below delta (standard MSE behaviour in the
normal operating range) and linear for residuals above delta (robust MAE
behaviour for the remaining tail).  This prevents the squared penalty from
dominating the gradient updates when the occasional large residual occurs,
whilst still providing the smooth gradient of MSE for typical samples.

Algorithm Reference
-------------------
NiOA is inspired by the adaptive hunting strategies of ninjas, wherein agents
alternate between broad environmental reconnaissance (exploration) and directed
movement towards the highest-value target (exploitation).
...
class NinjaOptimizationAlgorithm:
    ...
    and iteratively refines them through alternating exploration and
    exploitation phases.
    ...
    max_iterations       : int       Refinement iterations after initialisation.
    ...
    def _exploration_phase(self, agent: Dict, iteration: int) -> Dict:
        """
        Perturb agent parameters with magnitude decaying as optimisation
        progresses (broad early, narrow late).
        """
    ...
    def _exploitation_phase(
        ...
        """
        Move each agent towards the current best configuration, with
        attraction strength increasing as iterations advance.
        """
        new_agent = {}
        progress  = iteration / self.max_iterations
    ...
    def optimize(self):
        ...
        best_fitness      : float   Minimum validation loss achieved.
        convergence_curve : List    Best fitness after each iteration.
        """
        print("Starting Ninja Optimisation Algorithm...\n")
        start_time = time.time()

        # Evaluate initial randomly sampled population
        ...
        if self.best_agent is None:
            raise RuntimeError(
                ...
                "successfully evaluated.  Please inspect the objective "
                "function for runtime errors."
            )
...
def create_lstm_model(
    ...
    loss_fn    : str    Loss function: 'huber', 'mae', or 'mse'.
                        Default is 'huber' (see module-level docstring for
                        rationale).
    huber_delta : float Delta parameter for Huber loss.  Ignored when
                        loss_fn is 'mae' or 'mse'.
    ...
    # ------------------------------------------------------------------
    # Loss function selection  [CHANGED in v2]
    #
    # Previous version used 'mse' unconditionally.  MSE is highly sensitive
    # to outliers because it squares the residual, causing the gradient to
    # be dominated by a small number of large-error samples even after log1p
    # transformation.  Huber loss (the new default) switches to a linear
    # penalty beyond delta, providing robustness whilst preserving smooth
    # gradients in the typical operating range.
    # ------------------------------------------------------------------
    loss_fn_lower = loss_fn.lower().strip()

    if loss_fn_lower == "huber":
        compiled_loss = tf.keras.losses.Huber(delta=huber_delta)
    elif loss_fn_lower == "mae":
        compiled_loss = "mae"
    else:
        # Fallback to MSE (for ablation studies)
        compiled_loss = "mse"
